...
var getTypeRegExpChecker_1 = require("./getTypeRegExpChecker");
var getTypeCheckDiagnostics_1 = require("./getTypeCheckDiagnostics");
...
function run(config) {
    ...
    if (sources.length) {
        var typeRegExpChecker = getTypeRegExpChecker_1.getTypeRegExpChecker(config.regExpChecker);
        var diagnostics = getTypeCheckDiagnostics_1.getTypeCheckDiagnostics(checker, typeRegExpChecker, sources);
        emitter_1.emitter(diagnostics, config);
        reporter_1.reporter(diagnostics, config);
        result_1.result(diagnostics);
    }
}
exports.run = run;
